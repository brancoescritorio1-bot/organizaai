...
var DEFAULT_SUPABASE_URL = "https://gymxdeijrgorugqqiteh.supabase.co";
var DEFAULT_SUPABASE_ANON_KEY = "sb_publishable_" + "KPeLuuPs7mOf395Jv0YmeQ_mueP5jXE";
var DEFAULT_SUPABASE_SERVICE_ROLE_KEY = "sb_secret_" + "lXfZEvYWdg2WjYedXnwl8Q_qURe-vX7";
var supabaseUrlRaw = process.env.SUPABASE_URL || process.env.VITE_SUPABASE_URL || DEFAULT_SUPABASE_URL;
var supabaseUrl = (supabaseUrlRaw ? supabaseUrlRaw.split("/rest/v1/")[0].replace(/\/+$/, "") : "") || DEFAULT_SUPABASE_URL;
var geminiApiKey = process.env.GEMINI_API_KEY || process.env.VITE_GEMINI_API_KEY;
...
var supabaseKey = process.env.SUPABASE_ANON_KEY || process.env.VITE_SUPABASE_ANON_KEY || DEFAULT_SUPABASE_ANON_KEY;
var serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.VITE_SUPABASE_SERVICE_ROLE_KEY || DEFAULT_SUPABASE_SERVICE_ROLE_KEY;
var maskSecretSecurely = (key) => {
...
console.log("==========================================================");
var supabase = createClient(supabaseUrl, serviceRoleKey || supabaseKey, {
  auth: {
...
app.get("/api/users", async (req, res) => {
  try {
    const serviceRoleKey2 = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.VITE_SUPABASE_SERVICE_ROLE_KEY;
    if (!serviceRoleKey2) {
    ...
    try {
      const payload = JSON.parse(Buffer.from(serviceRoleKey2.split(".")[1], "base64").toString());
    ...
    let users = [];
    const useDirectFetch = !serviceRoleKey2.startsWith("eyJ");
    if (useDirectFetch) {
      ...
      const supabaseAdmin = createClient(supabaseUrl, serviceRoleKey2, {
        auth: {
          autoRefreshToken: false,
          persistSession: false
        }
...
app.post("/api/subjects", async (req, res) => {
  ...
  const { data: subject, error: sError } = await supabase.from("subjects").insert([insertData]).select().single();
  if (sError) {
...
app.put("/api/subjects/:id", async (req, res) => {
  ...
  const { error } = await supabase.from("subjects").update(updateData).eq("id", req.params.id).eq("user_id", user.id);
  if (error) {
...
app.delete("/api/subjects/:id", async (req, res) => {
  const user = req.user;
  const { error } = await supabase.from("subjects").delete().eq("id", req.params.id).eq("user_id", user.id);
  if (error) {
...
app.put("/api/attendance/:subjectId", async (req, res) => {
  ...
  const { data: updated, error: updateError } = await supabase.from("presencas").update(updateData).eq("mes_materia_id", subjectId).select();
  if (updateError) {
...
app.put("/api/activities/:subjectId", async (req, res) => {
  ...
  const { data: updated, error: updateError } = await supabase.from("notas_atividades").update(updateData).eq("mes_materia_id", subjectId).select();
  if (updateError) {
...
app.put("/api/web_contents/:subjectId", async (req, res) => {
  ...
  const updateData = {};
  const fields = [
    "conteudo_1_assistido",
    "conteudo_2_assistido",
    "conteudo_3_assistido",
    "conteudo_4_assistido",
    "data_1",
    "data_2",
    "data_3",
    "data_4",
  ...
  const { data: updated, error: updateError } = await supabase.from("conteudos_web").update(updateData).eq("mes_materia_id", subjectId).select();
  if (updateError) {
...
app.get("/api/dashboard", async (req, res) => {
  try {
    ...
    const flattened = subjectsData.map((item) => {
      ...
      return {
        ...
        act1_status: nota?.atividade_1_status || "N\xE3o iniciada",
        act1_grade: nota?.atividade_1_nota,
        act2_status: nota?.atividade_2_status || "N\xE3o iniciada",
...
app.post("/api/setup-finance", async (req, res) => {
  ...
  res.json({
    success: true,
    tables: status,
    message: "Verifica\xE7\xE3o conclu\xEDda. Se alguma tabela estiver faltando (false), por favor execute o script SQL no painel do Supabase."
  });
});
...
app.post("/api/finance/transactions", async (req, res) => {
  ...
  if (is_installment && total_installments > 1) {
    ...
    const installmentSplits = (splits || []).map((s) => ({
      ...s,
    ...
    for (let i = 2; i <= total_installments; i++) {
      ...
      installments.push({
        ...
        status: "pendente",
        // Subsequent installments are usually pending
        user_id: user.id,
        is_installment: true,
        ...
        due_date: (() => {
          ...
          const d = /* @__PURE__ */ new Date(due_date + "T12:00:00");
...
app.put("/api/finance/transactions/:id", async (req, res) => {
  ...
  const { error } = await supabase.from("financial_transactions").update(updateData).eq("id", req.params.id).eq("user_id", user.id);
  if (error) return res.status(500).json(error);
  res.json({ success: true });
});
app.delete("/api/finance/transactions/:id", async (req, res) => {
  ...
  const { data: transaction, error: fetchError } = await supabase.from("financial_transactions").select("id, is_installment, parent_transaction_id").eq("id", id).eq("user_id", user.id).single();
  ...
  const { error } = await supabase.from("financial_transactions").delete().eq("id", idToDelete).eq("user_id", user.id);
  if (error) return res.status(500).json(error);
  res.json({ success: true });
});
...
app.get("/api/clients", async (req, res) => {
  ...
  const processedData = (data || []).map((client) => {
    ...
    const lastPurchase = sales.length > 0 ? sales.reduce(
      (latest, current) => current.purchase_date > latest ? current.purchase_date : latest,
...
app.get("/api/client-sales", async (req, res) => {
  const user = req.user;
  const { data, error } = await supabase.from("client_sales").select(`
        *,
        clients (name)
...
app.get("/api/client-installments", async (req, res) => {
  ...
  let query = supabase.from("client_installments").select(`
        *,
        client_sales (
          description,
          installment_count,
      ...
      `).eq("user_id", user.id).order("due_date");
  if (month && year) {
  ...
  if (client_id) {
    filteredData = filteredData.filter((item) => item.client_sales?.client_id === Number(client_id));
  }
...
app.put("/api/client-installments/:id", async (req, res) => {
  ...
  if (payment_date !== void 0) updateData.payment_date = payment_date;
  if (amount !== void 0) updateData.amount = amount;
  if (due_date !== void 0) updateData.due_date = due_date;
  const { error } = await supabase.from("client_installments").update(updateData).eq("id", req.params.id).eq("user_id", user.id);
  if (error) return res.status(500).json(error);
  res.json({ success: true });
});
app.delete("/api/client-installments/:id", async (req, res) => {
  const user = req.user;
  const { error } = await supabase.from("client_installments").delete().eq("id", req.params.id).eq("user_id", user.id);
  if (error) return res.status(500).json(error);
  res.json({ success: true });
});
app.get("/api/marketing/clients", async (req, res) => {
  try {
    const user = req.user;
    const { data, error } = await supabase.from("marketing_clients").select("*").eq("user_id", user.id).order("name");
    if (error) {
...
app.put("/api/marketing/clients/:id", async (req, res) => {
  try {
    const user = req.user;
    const { name, company, phone, plan_name, plan_value, status, publication_days } = req.body;
    const { error } = await supabase.from("marketing_clients").update({ name, company, phone, plan_name, plan_value, status, publication_days }).eq("id", req.params.id).eq("user_id", user.id);
    if (error) return res.status(500).json(error);
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.delete("/api/marketing/clients/:id", async (req, res) => {
  try {
    const user = req.user;
    const { error } = await supabase.from("marketing_clients").delete().eq("id", req.params.id).eq("user_id", user.id);
    if (error) return res.status(500).json(error);
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.get("/api/marketing/payments", async (req, res) => {
  try {
    const user = req.user;
    const { data, error } = await supabase.from("marketing_payments").select(`
          *,
...
app.put("/api/marketing/payments/:id", async (req, res) => {
  try {
    const user = req.user;
    const { client_id, month_reference, amount, due_date, payment_date, status } = req.body;
    const { error } = await supabase.from("marketing_payments").update({ client_id, month_reference, amount, due_date, payment_date, status }).eq("id", req.params.id).eq("user_id", user.id);
    if (error) return res.status(500).json(error);
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.delete("/api/marketing/payments/:id", async (req, res) => {
  try {
    const user = req.user;
    const { error } = await supabase.from("marketing_payments").delete().eq("id", req.params.id).eq("user_id", user.id);
    if (error) return res.status(500).json(error);
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.get("/api/marketing/posts", async (req, res) => {
  try {
    const user = req.user;
    const { data, error } = await supabase.from("marketing_posts").select("*").eq("user_id", user.id).order("scheduled_date", { ascending: true });
    if (error) {
...
app.put("/api/marketing/posts/:id", async (req, res) => {
  try {
    const user = req.user;
    const { title, scheduled_date, scheduled_time, social_network, status, caption, attachment_url, client_id } = req.body;
    const { error } = await supabase.from("marketing_posts").update({ title, scheduled_date, scheduled_time, social_network, status, caption, attachment_url, client_id: client_id || null }).eq("id", req.params.id).eq("user_id", user.id);
    if (error) return res.status(500).json(error);
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.delete("/api/marketing/posts/:id", async (req, res) => {
  try {
    const user = req.user;
    const { error } = await supabase.from("marketing_posts").delete().eq("id", req.params.id).eq("user_id", user.id);
    if (error) return res.status(500).json(error);
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.post("/api/ai/generate-caption", async (req, res) => {
  try {
    if (!ai) {
      return res.status(501).json({ error: "O servi\xE7o Gemini AI n\xE3o est\xE1 configurado ou a chave de API est\xE1 ausente no servidor." });
    }
    const { title, social_network, context } = req.body;
    if (!title) {
      return res.status(400).json({ error: "Por favor, informe o t\xEDtulo ou tema do post." });
    }
    console.log(`Generating AI caption with Gemini for post: "${title}" (${social_network})`);
    const prompt = `Voc\xEA \xE9 um especialista em marketing digital e reda\xE7\xE3o publicit\xE1ria (copywriting).
Escreva uma legenda atraente, persuasiva e altamente profissional em portugu\xEAs para uma postagem com o seguinte tema/t\xEDtulo: "${title}".
Rede Social de destino: ${social_network || "Instagram"}.
Contexto adicional ou p\xFAblico-alvo: ${context || "Geral"}.

Diretrizes da legenda:
1. Comece com um gancho forte (headline) para prender a aten\xE7\xE3o de imediato.
2. Divida o texto em par\xE1grafos curtos para tornar a leitura escane\xE1vel e agrad\xE1vel.
3. Inclua uma chamada para a\xE7\xE3o (CTA) marcante e apropriada no final (ex: "Deixe seu coment\xE1rio", "Acesse o link da nossa bio", "Compartilhe com quem precisa saber disso").
4. Adicione hashtags relevantes e estrat\xE9gicas ao final da legenda.
5. Use emojis de forma moderada e profissional para animar e humanizar a mensagem.

Retorne APENAS o texto da legenda formatada, sem coment\xE1rios extras, introdu\xE7\xF5es do tipo "Aqui est\xE1 a legenda" ou blocos de c\xF3digo formatados com crase.`;
    ...
    res.status(500).json({ error: err.message || "Erro ao gerar legenda com intelig\xEAncia artificial." });
  }
});
...
app.post("/api/chacara/users", async (req, res) => {
  ...
  const { data, error } = await supabase.from("chacara_users").insert([{
    ...
    phone,
    user_id: user.id,
    has_energy: has_energy !== void 0 ? has_energy : true,
    has_water: has_water !== void 0 ? has_water : true,
    energy_meters_count: energy_meters_count || 1,
    water_meters_count: water_meters_count || 1,
    energy_active: energy_active !== void 0 ? energy_active : true,
    water_active: water_active !== void 0 ? water_active : true
  }]).select().single();
  if (error) return res.status(500).json(error);
  res.json(data);
});
...
app.post("/api/chacara/bills", async (req, res) => {
  ...
  const {
    ...
    water_prev_reading,
    water_curr_reading,
    water_prev_reading_2,
    water_curr_reading_2,
    water_value,
    water_service_fee,
    apportionment_value,
...
app.get("/api/chacara/settings", async (req, res) => {
  ...
  if (error) {
    if (error.code === "42P01") {
      return res.status(500).json({ error: "A tabela 'chacara_settings' n\xE3o existe no banco de dados. Por favor, execute o script SQL no Supabase." });
    }
    return res.status(500).json(error);
  }
...
app.put("/api/chacara/settings", async (req, res) => {
  ...
  const {
    default_kwh,
    default_water_value,
    default_water_service_fee,
    default_apportionment_value,
    default_due_day,
    default_reading_day,
    reserve_fund_value,
  ...
  let error;
  if (existing) {
    const res2 = await supabase.from("chacara_settings").update({
      default_kwh,
      default_water_value,
      default_water_service_fee,
      default_apportionment_value,
    ...
    const res2 = await supabase.from("chacara_settings").insert([{
      user_id: user.id,
      default_kwh,
      default_water_value,
      default_water_service_fee,
...
app.get("/api/chacara/accountability/:month", async (req, res) => {
  ...
  const { data: accs, error: accError } = await supabase.from("chacara_accountability").select("*").eq("user_id", user.id).eq("month_reference", month).limit(1);
  if (accError) {
    if (accError.code === "42P01") {
      return res.status(500).json({ error: "A tabela 'chacara_accountability' n\xE3o existe no banco de dados. Por favor, execute o script SQL no Supabase." });
    }
    return res.status(500).json(accError);
  }
  const accountability = accs?.[0] || null;
  const { data: bills, error: billsError } = await supabase.from("chacara_bills").select("*").eq("user_id", user.id).in("status", ["paid", "partial"]).gte("payment_date", `${month}-01`).lte("payment_date", `${month}-31`);
  if (billsError) {
  ...
  if (bills) {
    bills.forEach((bill) => {
      ...
      const energyConsumption = energyReadings.length > 0 ? energyReadings.reduce((acc, r) => acc + (r.curr - r.prev), 0) : bill.curr_reading - bill.prev_reading + ((bill.curr_reading_2 || 0) - (bill.prev_reading_2 || 0));
      const energia = energyConsumption * bill.kwh_value;
      const waterConsumption = waterReadings.length > 0 ? waterReadings.reduce((acc, r) => acc + (r.curr - r.prev), 0) : (bill.water_curr_reading || 0) - (bill.water_prev_reading || 0) + ((bill.water_curr_reading_2 || 0) - (bill.water_prev_reading_2 || 0));
...
app.post("/api/chacara/accountability", async (req, res) => {
  ...
  const {
    month_reference,
    prev_balance_reserve_fund,
    prev_balance_main_account,
    prev_balance_services,
    prev_balance_energy,
    prev_balance_water,
    total_collected,
    closing_date
  } = req.body;
  const { data: existings, error: checkError } = await supabase.from("chacara_accountability").select("id").eq("user_id", user.id).eq("month_reference", month_reference).limit(1);
  if (checkError) {
    if (checkError.code === "42P01") {
      return res.status(500).json({ error: "A tabela 'chacara_accountability' n\xE3o existe no banco de dados. Por favor, execute o script SQL no Supabase." });
    }
...
app.get("/api/chacara/expenses", async (req, res) => {
  ...
  if (error) {
    if (error.code === "42P01") {
      return res.status(500).json({ error: "A tabela 'chacara_expenses' n\xE3o existe no banco de dados. Por favor, execute o script SQL no Supabase." });
    }
    return res.status(500).json(error);
  }
...
app.post("/api/chacara/expenses", async (req, res) => {
  ...
  if (!month_reference || !description || amount === void 0) {
    return res.status(400).json({ error: "Campos obrigat\xF3rios faltando." });
  }
  try {
    let { data: accs, error: accError } = await supabase.from("chacara_accountability").select("id").eq("user_id", user.id).eq("month_reference", month_reference).limit(1);
    let acc = accs?.[0];
    if (accError) {
    ...
    if (!acc) {
      const { data: newAcc, error: createError } = await supabase.from("chacara_accountability").insert([{ user_id: user.id, month_reference }]).select("id").single();
      if (createError) {
...
app.get("/api/safety/reports", async (req, res) => {
  const user = req.user;
  const { data, error } = await supabase.from("safety_reports").select("*").eq("user_id", user.id).order("created_at", { ascending: false });
  if (error) {
...
app.get("/api/safety/reports/:id/non-conformities", async (req, res) => {
  const user = req.user;
  const { id } = req.params;
  const { data: report, error: rError } = await supabase.from("safety_reports").select("id").eq("id", id).eq("user_id", user.id).single();
  if (rError || !report) {
    return res.status(403).json({ error: "Unauthorized or not found" });
  }
  const { data, error } = await supabase.from("safety_non_conformities").select("*").eq("report_id", id).order("created_at", { ascending: true });
  if (error) {
...
app.post("/api/safety/reports", async (req, res) => {
  ...
  const { data: report, error: rError } = await supabase.from("safety_reports").insert([{
    user_id: user.id,
    report_number,
    location,
    supervisor,
  ...
  if (non_conformities && non_conformities.length > 0) {
    const ncs = non_conformities.map((nc) => ({
      report_id: report.id,
    ...
    const { error: nError } = await supabase.from("safety_non_conformities").insert(ncs);
    if (nError) {
...
app.put("/api/safety/reports/:id", async (req, res) => {
  ...
  const { data: report, error: rError } = await supabase.from("safety_reports").update({
    report_number,
    location,
    supervisor,
    status,
...
app.delete("/api/safety/reports/:id", async (req, res) => {
  const user = req.user;
  const { id } = req.params;
  const { error } = await supabase.from("safety_reports").delete().eq("id", id).eq("user_id", user.id);
  if (error) {
...
app.get("/api/personal/tasks", async (req, res) => {
  const user = req.user;
  const { data, error } = await supabase.from("personal_tasks").select("*").eq("user_id", user.id).order("due_date", { ascending: true, nullsFirst: false });
  if (error) return res.status(500).json(error);
  res.json(data);
});
app.post("/api/personal/tasks", async (req, res) => {
  ...
  const { data, error } = await supabase.from("personal_tasks").insert([{ user_id: user.id, title, description, due_date, priority, status, eisenhower_quadrant }]).select().single();
  if (error) return res.status(500).json(error);
  res.json(data);
});
app.put("/api/personal/tasks/:id", async (req, res) => {
  ...
  const { id } = req.params;
  const { title, description, due_date, priority, status, eisenhower_quadrant } = req.body;
  const { data, error } = await supabase.from("personal_tasks").update({ title, description, due_date, priority, status, eisenhower_quadrant }).eq("id", id).eq("user_id", user.id).select().single();
  if (error) return res.status(500).json(error);
  res.json(data);
});
...
async function setupVite() {
  ...
  if (!process.env.VERCEL) {
    app.listen(PORT, "0.0.0.0", async () => {
      ...
      console.log("\n\u{1F50D} Verificando tabelas no Supabase...");
      for (const table of tables) {
      ...
      console.log("\n\u{1F4A1} DICA: Se alguma tabela acima falhou, execute o script SQL no painel do Supabase.\n");
    });
  }
}
if (!process.env.VERCEL) {
